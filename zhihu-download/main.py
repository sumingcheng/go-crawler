...
def judge_zhihu_type(url, cookies=None, session=None, hexo_uploader=False):
    ...
    try:
        ...
        # 网络请求相关错误
        raise Exception(f"网络请求错误 - URL: {url} - {str(e)}")
    except Exception as e:
        # 其他错误
        raise Exception(f"处理错误 - URL: {url} - {str(e)}")
...
def save_and_transform(title_element, content_element, author, url, hexo_uploader, soup, date=None):
    ...
    # 修改日期格式：从 (日期) 改为 日期.
    if date:
        markdown_title = f"{date}.{markdown_title}"
    else:
    ...
    if content_element is not None:
        ...
        for img in content_element.find_all("img"):
            ...
            # 处理图片URL（去除缩放参数）
            if img_url.find("_r.jpg") != -1:
                img_url = img_url.split("_r.jpg")[0] + ".jpg"
            elif img_url.find("_b.jpg") != -1:
                img_url = img_url.split("_b.jpg")[0] + ".jpg"

            # 获取原始文件扩展名
            original_ext = os.path.splitext(img_url)[1].lower()
            # 如果没有扩展名或扩展名不在支持列表中，默认使用.jpg
            if not original_ext or original_ext not in ['.jpg', '.jpeg', '.png', '.gif']:
                original_ext = '.jpg'

            # 使用URL的哈希值作为文件名
            img_hash = get_file_hash(img_url)
            img_filename = f"{img_hash}{original_ext}"
            
            try:
                # 下载图片
                download_image(img_url, img_filename)
                # 替换为本地图片链接（使用相对路径）
                # 注意：这里直接使用图片文件名，因为markdown文件和图片在同一目录下
                img.replace_with(f"![{img_hash}]({img_filename})")
            except Exception as e:
